...
export const CODEBASE_ANALYSIS_SIGNATURE = f()
  ...
  .output(
    ...
    f.string("Architecture & Design Patterns"),
  )
  .output(
    "antiPatternsAndRestrictions",
    f.string("Anti-Patterns & Restrictions"),
  )
...
export const EXTRACT_AGENTS_SECTIONS_SIGNATURE = f()
  .input(
    "conventionsMarkdown",
    f.string(
      "The extracted architectural, data flow, and granular coding conventions",
    ),
  )
  ...
  .output(
    "techStack",
    f.string(
      "Explicit list of supported languages, frameworks, and tools used in the repository.",
    ),
  )
  .output(
    "architecture",
  ...
  .output(
    "antiPatternsAndRestrictions",
    f.string(
      "Specific anti-patterns and 'NEVER do this' rules the AI must strictly avoid.",
    ),
  )
  .output(
    "databaseAndState",
  ...
  .output(
    ...
    f.string(
      "Strict security guardrails, such as rules against exposing secrets or logging PII.",
    ),
  )
  .output(
  ...
  .output(
    ...
    f.string(
      "Standards for writing docstrings, comments, and updating system/user documentation.",
    ),
  )
  .output(
    "commonPatterns",
