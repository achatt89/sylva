...
export async function resolveRepositoryTarget(
  ...
  if (githubRepo) {
    ...
    let repoName =
      repoUrl.replace(/\/$/, "").split("/").pop() || "unknown-repo";
...
async function runPipeline(
  ...
  if (numFiles === 0) {
    ...
    console.log(
      `✅ Extracted representation for ${numFiles} top-level file(s)/directory(ies).`,
    );
  }
  ...
  const conventionsMarkdown = await extractor.compileMarkdown(
    llmMini,
    rlmResult,
  );
...
async function main() {
  ...
  program
    ...
    .argument(
      "[repo]",
      "Absolute path to a local repository to analyze (default)",
    )
    .option(
    ...
    .option(
      ...
      "Absolute path to a local repository to analyze",
    )
    ...
    .action(async (repo, options) => {
      ...
      try {
        const { repoUrl, localPath, repoName } =
          await resolveRepositoryTarget(parsedArgs);
        ...
        if (repoUrl) {
          targetDir = fs.mkdtempSync(
            path.join(process.cwd(), `sylva-tmp-${repoName}-`),
          );
