...
describe('cli utilities', () => {
    describe('resolveRepositoryTarget', () => {
        it('resolves an explicit github repository URL', async () => {
            const result = await resolveRepositoryTarget({ githubRepository: 'https://github.com/expressjs/express' });
        ...
        it('resolves a positional repo string as git if it starts with http', async () => {
            const result = await resolveRepositoryTarget({ repo: 'https://github.com/foo/bar.git' });
